...
def test_ner(epoch, name, results, path):
    """
    Run perl script to evaluate model
    """
    # print('results: %s' % results)
    file_name = name + str(epoch) + '_predict.utf8'
    ...
    with codecs.open(output_file, "w", 'utf8') as f:
        ...
        for block in results:
            ...
            to_write.append("\n")
        f.writelines(to_write)
    eval_lines = return_report(output_file)  # 进入conllevalue.py评测脚本
    return eval_lines
...
def create_model(session, Model_class, path, config, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        session.run(tf.global_variables_initializer()) #初始化模型的参数
    return model
...
def result_to_json(string, tags):
    ...
    for char, tag in zip(string, tags):
        if tag[0] == "S":
            ...
            # elif tag[2:] == "ANA":
            #     label = "解剖部位"
            item["entities"].append({"word": char, "start_pos": idx, "end_pos": idx+1, "label_type": label})
        elif tag[0] == "B":
            entity_name += char
            entity_start = idx
        elif tag[0] == "I":
            ...
            # elif tag[2:] == "ANA":
            #     label = "解剖部位"
            item["entities"].append({"word": entity_name, "start_pos": entity_start, "end_pos": idx + 1, "label_type": label})
            entity_name = ""
        else:
            entity_name = ""
            entity_start = idx
    ...
    for c_idx in range(len(tags)):
        ...
        if tag[0] == 'B':
            ...
            if tag_next[2:] != entity_tag:
                ...
                item["entities"].append({"word": c, "start_pos": iCount, "end_pos": iCount + 1, "label_type": label})
        elif tag[0] == "I":
            if tag[2:] != tags[c_idx-1][2:] or tags[c_idx-1][2:] == 'O':
                ...
                if tag_next[2:] != entity_tag:
                    ...
                    item["entities"].append({"word": entity_name, "start_pos": entity_start, "end_pos": iCount + 1, "label_type": label})
                    entity_name = ''
        iCount += 1
    return item
...
def convert_single_example(char_line, tag_to_id, max_seq_length, tokenizer, label_line):
    ...
    ntokens.append("[SEP]") #句尾添加SEP标志
    segment_ids.append(0)
    # append("O") or append("[SEP]") not sure!
    label_ids.append(tag_to_id["[SEP]"])
    input_ids = tokenizer.convert_tokens_to_ids(ntokens) #将序列中的字（ntokens）转化成ID形式（vocab.txt中对应的ID）
