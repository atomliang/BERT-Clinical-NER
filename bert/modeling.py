...
def embedding_lookup(input_ids,
  ...
  if use_one_hot_embeddings:
    ...
    output = tf.gather(embedding_table, flat_input_ids) # CPU,GPU运算1024,768一个batch里所有的映射结果
...
def attention_layer(from_tensor,
  ...
  if attention_mask is not None:
    ...
    attention_scores += adder#　把这个加入到原始的得分里相当于mask为１的就不变，mask为０的就会变成非常大的负数

  # Normalize the attention scores to probabilities.
  # `attention_probs` = [B, N, F, T]
  attention_probs = tf.nn.softmax(attention_scores)#　再做softmax此时负数做softmax相当于结果为０了就相当于不考虑了
