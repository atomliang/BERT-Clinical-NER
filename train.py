...
def evaluate(sess, epoch, model, name, data, id_to_tag, logger):
    logger.info("evaluate:{}".format(name))
    ner_results = model.evaluate(sess, data, id_to_tag)
    eval_lines = test_ner(epoch, name, ner_results, FLAGS.result_path)
...
def train():
    ...
    with tf.Session(config=tf_config) as sess:
        model = create_model(sess, Model, FLAGS.ckpt_path, config, logger)  # Model为模型
        ...
        for i in range(50):
            ...
            best = evaluate(sess, i+1, model, "dev", dev_manager, id_to_tag, logger)
            if best:
                save_model(sess, model, FLAGS.ckpt_path, logger, global_steps=step)
            evaluate(sess, i+1, model, "test", test_manager, id_to_tag, logger)
